...
func requiresHTTP1Tr(r *transportRequest) bool {
	return hasToken(r.Header.Get("Connection"), "upgrade") &&
		strings.EqualFold(r.Header.Get("Upgrade"), "websocket")
}
...
func (t *Transport) connectMethodForRequest(treq *transportRequest) (cm connectMethod, err error) {
	...
	cm.onlyH1 = requiresHTTP1Tr(treq)
	return cm, err
}
...
func (pconn *persistConn) addTLS(name string, trace *ClientTrace) error {
...
func writeSubset2(h http.Header, w io.Writer, exclude map[string]bool, trace *httptrace.ClientTrace) error {
	ws, ok := w.(io.StringWriter)
	if !ok {
		ws = stringWriter{w}
	}
...
func write2(h http.Header, w io.Writer, trace *httptrace.ClientTrace) error {
	return writeSubset2(h, w, nil, trace)
}
...
func write(r *http.Request, w io.Writer, usingProxy bool, extraHeaders http.Header, waitForContinue func() bool) (err error) {
	...
	if extraHeaders != nil {
		err = write2(extraHeaders, w, trace)
		if err != nil {
			return err
		}
	}
...
//}
func wantsClose(r *transportRequest) bool {
	if r.Close {
		return true
	}
	return hasToken(get(r.Header, "Connection"), "close")
...
func (pc *persistConn) roundTrip(req *transportRequest) (resp *http.Response, err error) {
	...
	if pc.t.DisableKeepAlives && !wantsClose(req) {
