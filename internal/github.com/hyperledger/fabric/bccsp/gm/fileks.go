...
func (ks *fileBasedKeyStore) storePrivateKey(alias string, privateKey interface{}) error {
	...
	if err != nil {
		fmt.Printf("Failed converting private key to PEM [%s]: [%s]", alias, err)
		return err
	}

	err = ioutil.WriteFile(ks.getPathForAlias(alias, "sk"), rawKey, 0700)
	if err != nil {
		fmt.Printf("Failed storing private key [%s]: [%s]", alias, err)
		return err
	}
...
func (ks *fileBasedKeyStore) storePublicKey(alias string, publicKey interface{}) error {
	...
	if err != nil {
		fmt.Printf("Failed converting public key to PEM [%s]: [%s]", alias, err)
		return err
	}
...
func (ks *fileBasedKeyStore) storeKey(alias string, key []byte) error {
	...
	if err != nil {
		fmt.Printf("Failed converting key to PEM [%s]: [%s]", alias, err)
		return err
	}

	err = ioutil.WriteFile(ks.getPathForAlias(alias, "key"), pem, 0700)
	if err != nil {
		fmt.Printf("Failed storing key [%s]: [%s]", alias, err)
		return err
	}
...
func (ks *fileBasedKeyStore) loadPrivateKey(alias string) (interface{}, error) {
	...
	fmt.Printf("loadPrivateKey : %s", path)
	fmt.Printf("Loading private key [%s] at [%s]...", alias, path)

	raw, err := ioutil.ReadFile(path)
	if err != nil {
		fmt.Printf("Failed loading private key [%s]: [%s].", alias, err.Error())
...
func (ks *fileBasedKeyStore) loadPublicKey(alias string) (interface{}, error) {
	...
	fmt.Printf("Loading public key [%s] at [%s]...", alias, path)

	raw, err := ioutil.ReadFile(path)
	if err != nil {
		fmt.Printf("Failed loading public key [%s]: [%s].", alias, err.Error())
...
func (ks *fileBasedKeyStore) loadKey(alias string) ([]byte, error) {
	...
	fmt.Printf("loadKey : %s", path)
	fmt.Printf("Loading key [%s] at [%s]...", alias, path)

	pem, err := ioutil.ReadFile(path)
	if err != nil {
		fmt.Printf("Failed loading key [%s]: [%s].", alias, err.Error())
	...
	if err != nil {
		fmt.Printf("Failed parsing key [%s]: [%s]", alias, err)
...
func (ks *fileBasedKeyStore) openKeyStore() error {
	...
	fmt.Printf("KeyStore opened at [%s]...done\n", ks.path)
